...
import { Carousels } from './carousel_settings.js';

export function GalleriesController() {
  ...
  function loadJsonGalleries(url) {
    ...
    $.getJSON(url)
      .done(function (data) {
        ...
        // if data loaded succesfully, turn on the progress bar to show progress
        $('#content-spinner .progress-bar-wrapper').fadeIn(250);
        galleryCode = getGallery(cached);
        $('#main-gallery>.grid').html(galleryCode);

        const thumbnailGalleryCode = buildThumbnailGallery(cached);
        ...
        $($imagesToCheckContainer)
          ...
          .fail(function () {
            console.log(
              brokenCount +
                ' of images have broken links. Check your image paths.',
            );
          })
          .progress(function (instance, image) {
            if (image.isLoaded) {
              $('#content-spinner .progress-bar').width(
      ...
      .fail(function (err) {
        // The End of .Done function()
        console.log('Error: ' + err.statusText);
        $('#content-spinner .spinner-msg')
          .addClass('spinner-msg-alert')
          .html(
            'Could not load the data gallery file.</br> Check console for more detail.',
          );
      })
      .always(function () {
    ...
    for (var i = 0; i < images.length; i++) {
      ...
      if (images[i].portfolio_description_en !== 'image') {
        txt +=
          ...
          gridclass +
          "' data-tag='" +
          images[i].datatag.trim().toLowerCase() +
          "' data-thumbnailportfolio='" +
          images[i].datatag.trim().toLowerCase() +
          "'>";
        txt +=
          "<div class='thumbnail-wrapper'><h3 class='thumbnail-title'>" +
          images[i].datatag.trim().toLowerCase() +
          '</h3>';
        txt += "<div class='thumbnail-image-wrapper'>";
        txt +=
          "<img src='" + images[i].lead_photo + "' alt=''></div></div></div>";
      }
  ...
  function buildPortfolioMenu(data) {
    ...
    data.forEach(function (portfolio) {
      ...
      htmlCode +=
        "<li class='nav-item'><a class='nav-link' data-thumbnailportfolio='" +
        cacheportfolio +
        "' href='#'><span>" +
        cacheportfolio +
        '</span></a></li>';
  ...
  function getGallery(cachedImgs) {
    ...
    var txt;
    var butMaxMin =
      "<div class = 'max-min-button'><i class='icon ion-ios-expand'></i><i class='icon ion-ios-contract'></i></div>";
    ...
    for (var i = 0; i < cachedImgs.length; i++) {
      ...
      if (cachedImgs[i].portfolio_description_en !== 'image') {
        ...
        if (cachedImgs[i].portfolio_description_en === '') {
          ...
          txt =
            "<div class='content-text-box'><h3>" +
            cachedImgs[i].datatag.trim() +
            '</h3><h4>' +
            cachedImgs[i].portfolio_year.trim() +
            '</h4>' +
            butMaxMin +
            '<p>' +
            cachedImgs[i].portfolio_description_en.trim() +
            '</p></div>';
          ...
          galleryHTMLCode +=
            ...
            gridclass +
            "' data-tag = '" +
            cachedImgs[i].datatag.trim().toLowerCase() +
            indexPortfolioImage +
            "'>" +
            txt +
            '</div>';
        ...
        txt =
          ...
          cachedImgs[i].image_path +
          "' alt='" +
          cachedImgs[i].image_alt +
  ...
  function getRandomImages(data, numOfRandomImages) {
    ...
    if (cachedCarouselImgs.length < numOfRandomImages) {
      ...
      console.log(
        'The actual number of all images is: ' +
          numOfRandomImages +
          ' which is lower than defined number of random images.',
      );
    ...
    function getRNDimages(numRND) {
      ...
      for (var i = 0; i < numRND; i++) {
        var num = Math.floor(
          (1 + (cachedCarouselImgs.length - 1)) * Math.random(),
        );
  ...
  function cacheGallery(data) {
    ...
    for (var portfolioi = 0; portfolioi < galleries.length; portfolioi++) {
      ...
      for (
        var imagei = 0;
        imagei < galleries[portfolioi].image.length;
        imagei++
      ) {
        if (imagei === 0) {
          cachedImages[index] = new Cacheimage();
          cachedImages[index].portfolio_description_en =
            galleries[portfolioi].portfolio_description_en;
          ...
          cachedImages[index].portfolio_year =
            galleries[portfolioi].portfolio_year;
