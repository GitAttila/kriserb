export const Site = (function () {
  ...
  function getSiteData(funcCB) {
    ...
    $.getJSON(URL)
      .done(function (data) {
        ...
        let socmediaHTML = getSocialMediaSnippet(contactInfo);
        $('[data-site="socialmedia"]').html(socmediaHTML);
        let contactHTML = getContactSnippet(data);
        $('[data-site="contactinfo"]').html(contactHTML);
        let pressHTML = getPressSnippet(data);
        $('[data-site="press"]').html(pressHTML);
        let bioHTML = getBioSnippet(data);
        $('[data-site="biography"]').html(bioHTML);
        let publicationsHTML = getPublicationsSnippet(data);
      ...
      .fail(function (jqxhr, textStatus, error) {
        ...
        $('#content-spinner .spinner-msg')
          ...
          .html(
            'Could not load the site data file.</br> Check console for more detail.',
          );
        funcCB();
      });
  }
  ...
  function getContactSnippet(data) {
    ...
    if (data.contactinfo) {
      ...
      htmlCode +=
        '<img src="' +
        data.contactinfo.qrcode.path +
        '" alt="' +
        data.contactinfo.qrcode.alt +
        '" >';
      htmlCode +=
        '</div></div><div class="contact-card contact-divider col-xs-12 col-sm-7 col-md-8">';
      ...
      $(data.contactinfo.phone).each(function (key, val) {
        ...
        if (val.icon !== '' && val.number !== '') {
          htmlCode +=
            '<p><i class="icon ' + val.icon + '"></i>' + val.number + '</p>';
        }
      });
      htmlCode += '</address>';
    }
  ...
  function getPressSnippet(data) {
    ...
    if (data.press) {
      if (data.press.record) {
        ...
        $(data.press.record).each(function (key, val) {
          ...
          if (key !== 0 && key % 2 === 0) {
            htmlCodeCouple =
              '<div class="row dotted-spaced-light row-press">' +
              htmlCodeCouple +
              '</div>';
    ...
    function buildPressSnippet(jsonRecord) {
      ...
      if (jsonRecord.link) {
        ...
        jsonRecord.link.icon = (jsonRecord.link.icon || 'ion-ios-open')
          .trim()
          .toLowerCase();
        jsonRecord.link.text = (jsonRecord.link.text || 'visit')
          .trim()
          .toUpperCase();
        if (jsonRecord.link.path) {
          link =
            '<a class="press-link" href="' +
            jsonRecord.link.path +
            '" target="_blank">';
          link +=
            '<i class="icon ' +
            jsonRecord.link.icon +
            '"></i> ' +
            jsonRecord.link.text +
            ' </a>';
        }
      }
      ...
      if (jsonRecord.title !== '' && jsonRecord.text !== '') {
        html +=
          '<div class="col-xs-12 col-sm-6"><div class="row"><div class="col-xs-12 col-lg-4">';
        html += img;
  ...
  function getBioSnippet(data) {
    ...
    if (data.biography) {
      if (data.biography.intro) {
        htmlCode +=
          '<div class="bio-row row"><div class="col-xs-12 col-sm-12 col-md-3">';
        htmlCode +=
          ...
          '">';
        htmlCode +=
          '</div><div class="col-xs-12 col-sm-12 col-md-9 dotted-spaced-light">';
      ...
      if (data.biography.education) {
        htmlCode +=
          '<div class="bio-row row"><div class="col-xs-12 col-sm-12 col-md-3"></div>';
        htmlCode +=
          '<div class="col-xs-12 col-sm-12 col-md-9 dotted-spaced-light">';
    ...
    $(records).each(function (key, val) {
      ...
      if (val.recordlink !== '') {
        link =
          '<a href="' +
          val.recordlink +
          '" target="_blank"><i class="bio-link icon ion-ios-paper"></i></a>';
      }
    ...
    if (data.publications) {
      if (data.publications.record) {
        $(data.publications.record).each(function (key, val) {
          htmlCode +=
            '<div class="row dotted-spaced-light pub-item"><div class="col-12 col-sm-5">';
          htmlCode +=
            '<div id="publication-carousel_' +
            key +
            '" class="carousel carousel-pub slide data-ride="carousel">';
          ...
          htmlCode += '</div>';
          htmlCode +=
            '<a class="carousel-control-prev" href="#publication-carousel_' +
            key +
            '" role="button" data-slide="prev">';
          htmlCode +=
            '<span aria-hidden="true"><i class="icon ion-ios-arrow-back"></i></span><span class="sr-only">Previous</span></a>';
          htmlCode +=
            '<a class="carousel-control-next" href="#publication-carousel_' +
            key +
            '" role="button" data-slide="next">';
          htmlCode +=
            '<span aria-hidden="true"><i class="icon ion-ios-arrow-forward"></i></span><span class="sr-only">Next</span></a>';
          htmlCode += '</div></div>';
          ...
          if (val.btn && val.btn.link !== '') {
            htmlCode +=
              '<a href="' +
              val.btn.link +
              '" target="_blank" class="btn-site" role="button">';
