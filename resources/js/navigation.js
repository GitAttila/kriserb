export function Navigation(imgs) {
  ...
  function initNavigationListeners() {
    const isMobileFlag = this.isMobile.any() || false;
    ...
    $('[data-menulink].nav-link').on('click', function (e) {
      ...
      if (
        ...
        $(this).siblings('.submenu').length === 0
      ) {
        return;
      }
      ...
      if (
        $(this).children('span').children('.icon-menu-down').length > 0 ||
        $(this).children('span').children('.icon-menu-up').length > 0
      ) {
      ...
      if (linkTo !== 'about') {
        ...
        $('#navbar [data-menulink="' + linkTo + '"]')
          .parent()
          .addClass('active');
        const linkFrom = $('.section-content.active').attr('id').split('-')[1];
    ...
    $('[data-thumbnailportfolio]').on('click', function (e) {
      const filterTag = $(this).data('thumbnailportfolio');
      // autoclose the navbar manu after the selection has been made
      if (isMobileFlag) {
        $('#navbar').collapse('hide');
      }
      ...
      $('#navbar [data-thumbnailportfolio="' + filterTag + '"]')
        .parent()
        .addClass('active');
    ...
    $('.grid').on('click', '.grid-item', function () {
      var _self = this;
      const strToParse = $(this).data('tag');
      const slideNum = Number(strToParse.replace(/[^0-9]/g, ''));
  ...
  function makeSectionTransition(
    transitionFrom,
    transitionTo,
    animFrom,
    animTo,
      ...
      if (cachedImgs[i].datatag.trim().toLowerCase() === portName) {
        ...
        if (index > 0) {
          htmlCode +=
            ...
            "'></div>";
          indicators +=
            "<li data-target='#portfolio-carousel' data-slide-to='" +
            (index - 1) +
            "' " +
            indicator +
            '></li>';
  ...
  function getActivePortfolioName() {
    return $('#portfolios-menu .nav-item.active')
      .children()
